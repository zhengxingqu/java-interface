          ...
          <el-main>
              ...
              <el-row>
                <el-col :span="8">
                  <el-form-item label="请输入查询条件">
                    <el-input placeholder="输入查询条件" v-model="moduleName"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-button-group>
                    <el-button type="primary" icon="el-icon-search" @click="searchModule()">查询</el-button>
                    <el-button type="primary" icon="el-icon-share" @click="getPageModule()">全部</el-button>
            ...
            <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <el-button type="success" size="mini" @click="updateModule(scope.row)">编辑</el-button>
                  <el-button type="info" size="mini" @click="deleteModule(scope.row)">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
            ...
            >
              <el-form style="margin-left: 20px;" :inline="true" label-width="80px" label-position="right"
                       :model="moduleForm" :rules="rules" ref="moduleForm" class="demo-ruleForm">
                <!--                <el-upload-->
                ...
                <el-form-item label="模块名称" prop="moduleName">
                  <el-input suffix-icon="el-icon-edit" v-model="moduleForm.moduleName"
                  ></el-input>
                </el-form-item>
                <span style="color: red">*</span> 项目名称：<el-select v-model="moduleForm.projectName" placeholder="请选择">
                <el-option
              ...
              <span slot="footer" class="dialog-footer">
                    ...
                    >确 定</el-button>
            <el-button @click="closeDialog('moduleForm')">取 消</el-button>
        ...
        methods: {
            ...
            getPageModule() {
                ...
                that.$axios.get("/module/all", {
                    ...
                    console.log(res)
                    that.pageModuleList = res.data.data.data
            ...
            addModule() {
                ...
                that.$axios.get("/project/all", {
                    ...
                    that.projectList = res.data.data.data;
                }).catch(function (err) {
            ...
            editModule() {
                let that = this;
                that.$axios.put("/module/update/" + that.moduleForm.id, that.moduleForm).then(function (res) {
            ...
            createModule() {
                let that = this;
                that.$axios.post("/module/add", that.moduleForm).then(function (res) {
                    if (res.data.code === 20000) {
                        that.$message({
                            message: res.data.message,
                            type: 'success'
            ...
            deleteModule(row) {
                let that = this;
                that.moduleForm = JSON.parse(JSON.stringify(row));
                that.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    that.$axios.delete('/module/delete/' + that.moduleForm.id).then(function (res) {
                        if (res.data.code === 20000) {
                            that.$message({
                                type: 'success',
                                message: res.data.message
